      ...
      const markAsDone = () => {
        requests.map((user, index) => {
          if (
            ...
            const studentId = checked[index].studentId;
            const requestId = checked[index].requestId;
            fetch("http://localhost:4000/help-requests", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
        ...
        updatedRequests = changeIndex.map((item) => {
        ...
        const updatedChecked = changed.map((item) => {
          return { studentId: item.studentId, requestId: item.requestId, checked: false };
        });
